...
func (this *BaseController) Trace() {
	this.Trace()
}
...
//登录状态验证
func (this *BaseController) authSource() {
	...
	if this.controllerName == "ptask" && this.actionName == "restart" {
		return
	}
...
func (this *BaseController) auth() {
	...
	if len(arr) == 2 {
		...
		if userId > 0 {
			user, err := models.UserGetById(userId)
			if err == nil && password == libs.GetCookieAuthKey(user.Password, user.Salt) {
				this.userId = user.Id
				this.userName = user.UserName
				this.user = user
			}
...
func (this *BaseController) getSsoUrl(relativeUrl string,  params map[string]string) string {
	ssoBaseUrl := beego.AppConfig.String("ssoCenter.baseUrl")
	ssoUrl := ssoBaseUrl + "/" + relativeUrl + "/"
	
	return ssoUrl + libs.BuildUrlParams(params)
}

func (this *BaseController) getHost() string {
	var scheme = "http://"
	if this.Ctx.Request.TLS != nil {
		scheme = "https://"
	}

	return scheme + this.Ctx.Request.Host + "/"
}
